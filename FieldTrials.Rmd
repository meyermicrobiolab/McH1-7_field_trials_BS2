...
# First, replace 0 values with an estimate and perform CLR
d.clr <- microbiome::transform(otu, "clr")
...
sample_data(ps5_BS2)$Treatment<-factor(sample_data(ps5_BS2)$Treatment,levels=c("probiotic bag","probiotic paste","control bag","control paste","none","resistant"))
sample_data(ps5_BS2)$Collection.month<-factor(sample_data(ps5_BS2)$Collection.month,levels=c("Aug","Oct","Jan"))
...
p <- ggplot(df_out, aes(x = PC1, y = PC2, fill = samples$Treatment, shape = samples$Month)) +
  ...
  labs(x = xlab, y = ylab, fill = "Treatment", shape = "Month")+
