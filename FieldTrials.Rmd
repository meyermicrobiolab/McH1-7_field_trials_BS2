...
pdf("PCA_Aug.pdf", width = 8.5)
p1<-ggplot(df_out.aug,aes(x=PC1,y=PC2,color=samples$Treatment))
...
pdf("PCA_Oct.pdf", width = 8.5)
p2<-ggplot(df_out.oct,aes(x=PC1,y=PC2,color=samples$Treatment))
...
# permanova between groups using Aitchison distance
dist.clr <- dist(E.clr)
perm<-adonis2(dist.clr~Treatment,as(sample_data(ps5_BS2_oct),"data.frame"))
print(perm)
...
adonis2(formula = dist.clr ~ Treatment, data = as(sample_data(ps5_BS2_oct), "data.frame"))
          Df SumOfSqs      R2      F Pr(>F)
Treatment  5    10255 0.07159 1.0024  0.409
Residual  65   133001 0.92841              
...
pdf("PCA_Jan.pdf", width = 8.5)
p3<-ggplot(df_out.jan,aes(x=PC1,y=PC2,color=samples$Treatment))
...
# permanova between groups using Aitchison distance
dist.clr <- dist(E.clr)
perm<-adonis2(dist.clr~Treatment,as(sample_data(ps5_BS2_jan),"data.frame"))
print(perm)
```
...
adonis2(formula = dist.clr ~ Treatment, data = as(sample_data(ps5_BS2_jan), "data.frame"))
          Df SumOfSqs      R2     F Pr(>F)
Treatment  5    14066 0.08698 1.086  0.277
Residual  57   147653 0.91302             
...
ylab <- paste("PC2: ", pc2, sep="")
df_out.aug <- as.data.frame(d.pcx$x)
p1<-ggplot(df_out.aug,aes(x=PC1,y=PC2,color=samples.aug$Treatment))
p1<-p1+geom_point(size=2)+
  theme(axis.title = element_text(size=14))+
  theme(axis.text=element_text(size=12))+
  theme(legend.title = element_text(size=14))+
...
ylab <- paste("PC2: ", pc2, sep="")
df_out.oct <- as.data.frame(d.pcx$x)
p2<-ggplot(df_out.oct,aes(x=PC1,y=PC2,color=samples.oct$Treatment))
p2<-p2+geom_point(size=2)+
  theme(axis.title = element_text(size=14))+
  theme(axis.text=element_text(size=12))+
  theme(legend.title = element_text(size=14))+
...
ylab <- paste("PC2: ", pc2, sep="")
df_out.jan <- as.data.frame(d.pcx$x)
p3<-ggplot(df_out.jan,aes(x=PC1,y=PC2,color=samples.jan$Treatment))
p3<-p3+geom_point(size=2)+
  theme(axis.title = element_text(size=14))+
  theme(axis.text=element_text(size=12))+
  theme(legend.title = element_text(size=14))+
...
plot_grid(panels, legend, ncol = 1)
panels
...
pdf("barchart_Class_2.pdf",width=11)
