...
ps10_BS2<-filter_taxa(psBS2, function(x) mean(x) >10, TRUE)
ntaxa(ps10_BS2) #392
get_taxa_unique(ps10_BS2, "Order") #66
get_taxa_unique(ps10_BS2, "Class") #27
...
otu <- read.table("silva_nochloronomito_otu_table_ps5nb.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("silva_nochloronomito_taxa_table_ps5nb.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("metadata_ps5nb.txt",sep="\t",header=T,row.names=1)
OTU = otu_table(otu, taxa_are_rows=FALSE)
taxon<-as.matrix(taxon)
TAX = tax_table(taxon)
sampledata = sample_data(samples)
ps5nb <- phyloseq(otu_table(otu, taxa_are_rows=FALSE), 
               sample_data(samples), 
               tax_table(taxon))
ps5nb #470 taxa and 410 samples
