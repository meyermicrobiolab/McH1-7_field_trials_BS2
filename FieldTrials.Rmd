...
# read in otu table with graph-appropriate ASV names
asv <- read.table("silva_nochloronomito_otu_table_ps20_v3.txt",sep="\t",header=TRUE, row.names=1)
...
names(asv_long)[names(asv_long)=="value"] <- "relative abundance"
...
# color palatte for bactrial class
# sort ASVs by taxonomy

#make bubble plot with facte grid: horizontal=time, vertical=treatment
bubble <- ggplot(asv_meta, aes(sample,ASV))+
  geom_point(aes(size="relative abundance"),alpha=0.8)+ # the alpha value is the opacity of the bubble
  scale_size(range = c(0,40), name="relative abundance")+ #adjust the two numbers here to change bubble sizes
  theme_bw()+
