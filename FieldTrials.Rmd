...
# make sure there are no taxa with all zeros
ps5_BS2 <- prune_taxa(taxa_sums(ps5_BS2) > 1, ps5_BS2) 
ps5_BS2 #756 taxa and 194 samples

ps_ra<-transform_sample_counts(ps5_BS2, function(OTU) OTU/sum(OTU))
...
ps_ra_c = tax_glom(ps_ra, "Class")
ps_ra_c # 46 taxa and 194 samples
otu_c = as(otu_table(ps_ra_c), "matrix")
taxon_c = as(tax_table(ps_ra_c), "matrix")
cla<-t(otu_c)
cla <- as.data.frame(cla)
cla <- tibble::rownames_to_column(cla, "ASV")
taxon_c <- as.data.frame(taxon_c)
taxon_c <- tibble::rownames_to_column(taxon_c, "ASV")
cla_tax <- merge(taxon_c,cla, by="ASV", all=FALSE)
cla_tax$Order<-NULL
cla_tax$Family<-NULL
cla_tax$Genus<-NULL
cla_tax$ASV<-NULL
# sort by total abundance
cla_tax <- cla_tax %>% mutate("total" = rowSums((cla_tax[,4:197]), na.rm = TRUE))  
cla_tax <- cla_tax[order(cla_tax$total, decreasing = TRUE),]
# keep only 25 most abundant classes out of 46
cla25 <- cla_tax[1:25,]
cla25$total<-NULL
class_long<-melt(cla25,value.name="proportion",variable.name="sample",id.vars=c("Kingdom","Phylum","Class"))

# Merge with metadata for plotting aesthetics
samples <- as.data.frame(samples)
samples <- tibble::rownames_to_column(samples, "sample")
classplot <- merge(class_long,samples,by="sample", all=TRUE)

# set order of families on y-axis based on sorted total abundance, list order: first in list = bottom of y-axis
classplot$Class<-factor(classplot$Class,levels=c("Fusobacteriia","OM190","Desulfobacteria","KD4-96","Bacteria","Phycisphaerae","Anaerolineae","Oligoflexia","Blastocatellia","Chloroflexia","Vampirivibrionia","Desulfobulbia","Polyangia","Campylobacteria","Desulfovibrionia","Acidimicrobiia","Planctomycetes","Bacilli","Verrucomicrobiae","Clostridia","Cyanobacteriia","Bacteroidia","Actinobacteria","Alphaproteobacteria","Gammaproteobacteria"))
classplot$sampled<-factor(classplot$sampled,levels=c("Before treatment","Two weeks after treatment","Three months after treatment"))
classplot$Treatment<-factor(classplot$Treatment,levels=c("probiotic bag","probiotic paste","control bag","control paste","none"))
classplot$Condition<-factor(classplot$Condition,levels=c("HH","HD","DD"))
classplot$Collection.month<-factor(classplot$Collection.month,levels=c("Aug","Oct","Jan"))


p <- ggplot(classplot, aes(sample, Class, fill= proportion)) + geom_tile()+
  facet_grid(.~sampled,scales="free",space="free")+
