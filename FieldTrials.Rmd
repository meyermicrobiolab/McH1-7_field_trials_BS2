...
### Use corncob to look for differential abundant taxa by the continuous variable of total tissue loss


```{r, echo=FALSE}
# use an ASV table that has been filtered to remove low-abundance ASVs, no blanks, added ASV numbers to taxa table
otu <- read.table("silva_nochloronomito_otu_table_ps5_BS2_jan.txt",sep="\t",header=TRUE, row.names=1)
...
# save the tissueJan.da output and add asv, p.adj values to it, and ultimately taxonomny info
tissueJan.da.models <- lapply(tissueJan.da$significant_models, extractmods)
names(tissueJan.da.models) <- tissueJan.da$significant_taxa

# Add ASVs to the taxonomy table and save the significant asvs
tax_table(ps5_BS2_jan)[,7] <- rownames(tax_table(ps5_BS2_jan))
sig.taxonomy.tissueJan <- as.data.frame(tax_table(ps5_BS2_jan)[tissueJan.da$significant_taxa,]) 

# Move the data from a list to a dataframe and add taxonomy info
tissueJan.da.models.df <- ldply(tissueJan.da.models, data.frame) %>% 
  left_join(sig.taxonomy.tissueJan, by = c(".id" = "ASV")) %>%
  mutate(genusasv = paste0(Genus, "_(", .id,")"))

### Save these data so you don't have to re-run the model ###
write.table(tissueJan.da.models.df, "DA_tissueJan.txt", sep="\t",row.names = FALSE)
